...
class UsersTest extends PlaySpecification with AbstractControllerTest  {
  ...
  "Create User Test" should {
      ...
      "Bad JSON. Return BadRequest" in new WithApplication() {
      ...
      "Forbidden" in new WithApplication() {
           val jsonRequest: JsValue = Json.obj("id" -> "new","name" -> "foobar", "email" -> "foo@email.com", "enabled_rubbish" -> true)
      ...
      "No password supplied" in new WithApplication() {
      ...
      "Password not strong enough" in new WithApplication() {
      ...
      "Unique constraints violation" in new WithApplication() {
  ...
  "Update User Test" should {
     ...
     "Forbidden" in new WithApplication() {
      ...
      "Invalid externalised ID. Return BadRequest" in new WithApplication() {
  ...
  "Get User Test" should {
      ...
      "User with 1 role" in new WithApplication() {
           val externalisedUserId = indirectReferenceMapper.convertInternalIdToExternalised(1)
  ...
  "Get Users Test" should {
      ...
      "Just return 1 user" in new WithApplication() {
           val externalisedUserId = indirectReferenceMapper.convertInternalIdToExternalised(1)
  ...
  "Delete User" should {
     ...
     "Invalid externalised ID. Return BadRequest" in new WithApplication() {
      ...
      "Foreign Key constraints violation" in new WithApplication() {
           val user = User(Some(1),"simon@email.com",Some("password"),"Simon",0,true)
           val externalisedUserId = indirectReferenceMapper.convertInternalIdToExternalised(1)
           val result: Future[Result] = executeAdminUserOperation { usersController => request => 
  ...
  private def executeUserOperation(op:  Users => FakeRequest[Unit] => Future[Result]): Future[Result] = {
      ...
      val usersController = new Users(userSession,userService,configuration,indirectReferenceMapper)
  ...
  "Change user password" should {
     ...
     "Invalid externalised user id" in new WithApplication() {
           val externalisedUserId = "RUBBISH"
  ...
  "Change my password" should {
     ...
     "Incorrect current password - InvalidLoginAttempt " in new WithApplication() {
           val encryptedPassword = PasswordCrypt.encrypt("mypassword").get
           val result: Future[Result] = executeUserOperation { usersController => request =>
     ...
     "Incorrect current password - AccountLocked " in new WithApplication() {
           val encryptedPassword = PasswordCrypt.encrypt("mypassword").get
           val result: Future[Result] = executeUserOperation { usersController => request =>
